            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    <tr v-for="(customer_order, index) in customer_orders" :key=index>
                        ...
                        <td>{{customer_order.confirm_time}}</td>
                        ...
                        <td><button @click="{id_confirm = customer_order.hex_id;}" type="button" data-toggle="modal" data-target="#confirmModal" class="btn btn-outline-success"><i class="fa-solid fa-circle-check"></i> Ship</button></td>
                    </tr> 
                </tbody>
            </table>
        </div>
        ...
        <div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fa-solid fa-truck-fast"></i> Confirm Shipping</h5>
                ...
                <div class="modal-body">
                    <div class="alert alert-success" role="alert">
                        Are you sure you want to confirm the order <strong>{{id_confirm}}</strong> is being delivered ?
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button @click="confirm" type="button" class="btn btn-success"><i class="fa-solid fa-check"></i> Confirm Shipping</button>
                </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup() {
        document.title = "Meta Shop - Admin Wait for Shipping";
    },
    mounted(){
        ...
        BaseRequest.get('api/admin-order/wait-ship')
        .then( (data) =>{
    ...
    methods:{
        ...
        fsortlatest:function(){
            ...
            const { emitEvent } = useEventBus();
            BaseRequest.get('api/admin-order/wait-ship?page='+this.pageN+'&search='+this.searchad+'&sort='+this.sortlatest)
            .then( (data) =>{
                this.customer_orders = data.customer_order.data;
                this.quantity = data.customer_order.total;
                emitEvent('eventUserSuccess','Show Orders Success !');
        ...
        clickCallback:function(pageNum){
            this.pageN = pageNum;
            const { emitEvent } = useEventBus();
            BaseRequest.get('api/admin-order/wait-ship?page='+pageNum+'&search='+this.searchad+'&sort='+this.sortlatest)
            .then( (data) =>{
                this.customer_orders = data.customer_order.data;
                this.quantity = data.customer_order.total;
                emitEvent('eventUserSuccess','Show Orders Success !');
        ...
        confirm:function(){
            const { emitEvent } = useEventBus();
            BaseRequest.get('api/admin-order/ship?hex_id='+this.id_confirm)
            .then( () =>{
